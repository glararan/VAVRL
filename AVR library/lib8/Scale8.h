...
namespace VAVRL
{
	...
	inline void nscale8_LEAVING_R1_DIRTY( uint8_t& i, uint8_t scale)
	{
	#if SCALE8_C == 1
	#if (SCALE8_FIXED == 1)
	...
	/// If you are doing several 'scale8_video's in a row, use this, and
	/// then explicitly call cleanup_R1.
	inline void nscale8_video_LEAVING_R1_DIRTY( uint8_t & i, uint8_t scale)
	{
	#if SCALE8_C == 1 || defined(LIB8_ATTINY)
	...
	inline void nscale8x2( uint8_t& i, uint8_t& j, uint8_t scale)
	{
	#if SCALE8_C == 1
	#if SCALE8_FIXED == 1
		uint16_t scale_fixed = scale + 1;
	...
	inline void nscale8x2_video( uint8_t& i, uint8_t& j, uint8_t scale)
	{
	#if SCALE8_C == 1
		uint8_t nonzeroscale = (scale != 0) ? 1 : 0;
	...
	inline uint16_t scale16( uint16_t i, uint8_t scale )
	{
	  #if SCALE16_C == 1
		uint16_t result;
	#if SCALE8_FIXED == 1
	...
	inline uint8_t dim8_lin(uint8_t x)
	{
		if(x & 0x80)
			return scale8(x, x);
		else
	...
	{
		return 255 - scale8(255 - x, 255 - x);
	}

	/// inverse of the dimming function, brighten a value
	inline uint8_t brighten8_video( uint8_t x)
	{
		return 255 - scale8_video(255 - x, 255 - x);
	}

	/// inverse of the dimming function, brighten a value
	inline uint8_t brighten8_lin( uint8_t x )
	{
		uint8_t ix = 255 - x;
